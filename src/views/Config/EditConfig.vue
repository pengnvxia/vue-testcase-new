<template>
    <div>
        ...
        <a-form-model :model="variableForm" ref="variablesruleForm" :rules="ruleForm" class="variableForm">
            <a-row>
                <a-col :span="12">
    ...
    export default class EditConfig extends Vue{
        ...
        private addKey(): void {
            this.variableForm.variablesList.forEach(function(value: Variable){
                value.key=value.variableId;
            })
        }

        private getDetails(): void {
            console.log()
            configDetail(Number(this.$route.params.id)).then(
                (result: any) => {
                    this.variableForm=result.retval;
                    this.addKey();
                },
                (err: any) => {
                    this.$message;
