...
<script lang="ts">
    ...
    import {dbDetail,editDatabase} from "@/services/database";
    ...
    export default class EditDatabase extends Vue{
        ...
        private mounted(): void {
            this.databaseDetail(Number(this.$route.params.id));
        }

        private databaseDetail(id: number): void{
            dbDetail(id).then(
                (result: any) => {
                    if (result.errcode === "0") {
                        this.databaseForm=result.retval;
                    }
                },
                (err: any) => {
                    this.$message;
        ...
        private submit(): void{
            ...
            ref.validate((valid: boolean) => {
                if (valid){
                    editDatabase(this.databaseForm).then(
                        (result: any) => {
                            if (result.errcode === "0") {
